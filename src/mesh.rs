...
use std::{ffi::CString, mem::size_of, os::raw::c_void, ptr};

// NOTE: without repr(C) the compiler may reorder the fields or use different
// padding/alignment than C. Depending on how you pass the data to OpenGL, this
// may be bad. In this case it's not strictly necessary though because of the
// `offset!` macro used below in setup_mesh()
#[repr(C)]
pub struct Vertex {
    // position
...
impl Mesh {
    ...
    unsafe fn setup_mesh(&mut self) {
        ...
        // A great thing about structs with repr(C) is that their memory layout is
        // sequential for all its items. The effect is that we can simply pass a
        // pointer to the struct and it translates perfectly to a glm::vec3/2 array
        // which again translates to 3/2 floats which translates to a byte
        // array.
